...
train_dataframe = pd.read_csv('dataset2.csv')

#train_labels = train_dataframe.class_label
#labels = list(set(train_labels))
#train_labels = np.array([labels.index(x) for x in train_labels])
train_features = train_dataframe.iloc[0:,:64]

train_features = np.array(train_features)

train_labels = train_dataframe.iloc[:,64:]
...
test_dataframe = pd.read_csv('featurevector.csv')
...
test_features = test_dataframe.iloc[:,:64]
